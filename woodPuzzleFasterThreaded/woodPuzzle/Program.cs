...
namespace woodPuzzle
{
    ...
    public class BlockPuzzle
    {
        ...
        public uint SolutionCount = 0; // track how many total solutions found

        public Dictionary<BlockLine.LineOrientation, Point> DeltaPoints = new Dictionary<BlockLine.LineOrientation, Point>();
        ...
        public BlockPuzzle(int[] blockLineOrder)
        {
            ...
            this.SetupOrientationDict();
        }

        private void SetupOrientationDict()
        {
            Point leftPoint = new Point(-1, 0, 0);
            Point rightPoint = new Point(1, 0, 0);
            Point upPoint = new Point(0, 0, 1);
            Point downPoint = new Point(0, 0, -1);
            Point frontPoint = new Point(0, -1, 0);
            Point backPoint = new Point(0, 1, 0);
            Point noOrientPoint = new Point(0, 0, 0);

            this.DeltaPoints.Add(BlockLine.LineOrientation.Left, leftPoint);
            this.DeltaPoints.Add(BlockLine.LineOrientation.Right, rightPoint);
            this.DeltaPoints.Add(BlockLine.LineOrientation.Up, upPoint);
            this.DeltaPoints.Add(BlockLine.LineOrientation.Down, downPoint);
            this.DeltaPoints.Add(BlockLine.LineOrientation.Front, frontPoint);
            this.DeltaPoints.Add(BlockLine.LineOrientation.Back, backPoint);
            this.DeltaPoints.Add(BlockLine.LineOrientation.NoOrientation, noOrientPoint);
        }
        ...
        private bool AddBlockLine(int blockLineIndex)
        {
            ...
            this.BlockLineList[blockLineIndex].EndPoint = this.BlockLineList[blockLineIndex].StartPoint +
                ((this.BlockLineList[blockLineIndex].NumBlocks - 1) * this.DeltaPoints[this.BlockLineList[blockLineIndex].Orientation]);
        ...
        public bool SpaceCheck(BlockLine blockLine)
        {
            ...
            Point unitStep = this.DeltaPoints[blockLine.Orientation];
        ...
        public static void Solve(int[] blockLineOrder)
        {
            object printLock = new Object(); // Used so multiple threads don't try to print solutions at once
            ...
            // Use C# 4.5+ Parallels to spin up a new thread per starting point
            Parallel.ForEach(startingPoints, startingPoint =>
            {
                ...
                while (blockPuzzle.BlockLineIndex >= 0)
                {
                    ...
                    if (blockAdded)
                    {
                        ...
                        if (solutionFound)
                        {
                            blockPuzzle.SolutionCount++;
                            lock (printLock) // Lock on this so that solutions get printed in their entirety
                            {
    ...
    public class BlockLine
    {
        ...
        public void SetUnset(bool requestedState)
        {
            ...
            // Get a delta point to step in the right direction
            Point unitStep = this.Puzzle.DeltaPoints[this.Orientation];
